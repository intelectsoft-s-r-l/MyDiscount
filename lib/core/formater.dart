...
class Formater {
    ...
    try {
      final base64ImageString = list.map((map) {
        if (map[index] != '' && map[index] != null) {
          final String photoNews = map[index]
              .toString()
              .replaceRange(0, map[index].toString().indexOf(',') + 1, '');
          final Uint8List data =const Base64Decoder().convert(photoNews);
          map[index] = data ?? Uint8List.fromList([]);
          return map;
        } else {
          final Uint8List data = const Base64Decoder().convert(
  ...
  List parseDateTimeAndSetExpireDate(List list, String index) {
    try {
      final parsedDate = list.map((map) {
        final int milisec = int.tryParse(map[index]
            .replaceAll('/Date(', '')
            .replaceAll('+0300)/', '')
            .replaceAll('+0200)/', '') as String);
    ...
    try {
      ...
      for (final dynamic key in keys) {
        ...
        if (CompanyName(company.name) ==
            CompanyName(map['Company'] as String)) {
